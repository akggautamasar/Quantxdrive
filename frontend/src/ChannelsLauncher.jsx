...
export default function ChannelsLauncher({ token }) {
  const [targets, setTargets] = useState([]); const [open, setOpen] = useState(false);
  useEffect(() => { if (!token) return; const refresh=()=>setTargets(Array.from(document.querySelectorAll(".desktop-sb nav, .mobile-sb nav"))); refresh(); const observer=new MutationObserver(refresh); observer.observe(document.body,{childList:true,subtree:true}); return()=>observer.disconnect(); },[token]);
